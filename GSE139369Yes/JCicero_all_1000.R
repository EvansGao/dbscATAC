...
input <- args$matrix
#input <- "30078704_tissue_SmallIntestine.GSE149683.mm10"
#mkdir("R")
#mkdir("hundred")
#mkdir("peak")
...
clean <- scATAC_matrix[Matrix::rowSums(scATAC_matrix) > 3,]
}
...
#write.table(rownames(clean),"names.txt",col.names=FALSE,sep="\t",quote=FALSE)
#write.table(rownames(scATAC_matrix),"namesscATAC_matrix.txt",col.names=FALSE,sep="\t",quote=FALSE)
...
clean <- clean[,1:1000]
clean[clean > 0] <- 1
clean <- clean[Matrix::rowSums(clean) > 10,]
}
...
wind1 <- 500000
wind2 <- 100000
}else if(species == "ChlSab1_1"){
#2.79 ¡Á 109 base-pairs
genomepath <- "/data/gts/dbscATAC/project/chromsizes/ChlSab1_1.chrom.sizes"
dist_const <- 240000
wind1 <- 450000
wind2 <- 90000
...
#2.4 ¡Á 109 base-pairs
genomepath <- "/data/gts/dbscATAC/project/chromsizes/B73v4.chrom.sizes"
dist_const <- 200000
wind1 <- 400000
wind2 <- 80000
