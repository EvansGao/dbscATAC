...
open $filename, "fragment/".$cluster.".bed";
open $SE,">tmp/".$cluster."tmp.bed";
while(<$filename>){
s/\r|\n//g;
@tmp=split/\t/,$_;
...
system("computeMatrix scale-regions -S tmp/".$cluster.".bigwig -R e/".$cluster."_enh.bed -a 10000 -b 10000 --regionBodyLength 1000 --maxThreshold 1000 --missingDataAsZero -out tmp/".$cluster."_E.tab.gz");
...
system("computeMatrix scale-regions -S tmp/".$cluster.".bigwig -R tmp/".$cluster."_SE.bed -a 10000 -b 10000 --binSize 200 --regionBodyLength 18000 --maxThreshold 1000 --missingDataAsZero -out tmp/".$cluster."_SE.tab.gz");
