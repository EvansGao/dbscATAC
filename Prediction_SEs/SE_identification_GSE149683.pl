...
#@celltypes=("GSM4508933_intestine_filtered_Stromal_cells.GSE149683.hg38");
print join("Good\n",@celltypes);
#print $file."\n";
...
for($ii=0;$ii<scalar(@celltypes);$ii++){
  ...
  }elsif(($pronum+1)%$piece!=0 && ($pronum+1)==scalar(@celltypes)){
...
{   
	my ($cell)=@_;
	system("Rscript ./JCicero_all_1000.R -m ".$cell);
}

sub   ALL()
{   
	my ($cluster)=@_;
#	system("Rscript ./JCicero_all_1000.R -m ".$cluster);
	system("bedtools sort -i fragmentnew/".$cluster.".bed>tmp/".$cluster."_sorted.bed");
#brain_ATAC_L5_PT.10X_MouseBrain.mm10.bed
...
#	system("mv tmp/".$cluster."/".$cellinfo[0]."_Plot_points.png SEfig");
	system("tar -zcvf fragmentnew/".$cluster.".bed.tar.gz fragmentnew/".$cluster.".bed");
	unlink("fragmentnew/".$cluster.".bed");
	system("sync; echo 3 > /proc/sys/vm/drop_caches");
#	unlink($cluster.".bed");
#	system("rm -rf tmp/".$cluster);
#GSM6598797_CD_C5

#tar -zcvf fragment/30078704_celltype_Cerebellum_Oligodendrocytes.GSE111586.mm10.bed.tar.gz fragment/30078704_celltype_Cerebellum_Oligodendrocytes.GSE111586.mm10.bed
#@celltypes=("30078704_tissue_Testes.GSE111586.mm10","30078704_celltype_Testes_Sperm.GSE111586.mm10","30078704_celltype_Liver_Endothelial_II_cells.GSE111586.mm10","30078704_celltype_Cerebellum_Oligodendrocytes.GSE111586.mm10");
